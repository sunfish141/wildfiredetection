...
| Weather tiles | 5.0 GB | Retain compact fixed-variable Open-Meteo ECMWF IFS historical-analysis tiles selected from candidate evidence, plus optional issued-forecast tiles for forward experiments. |
...
- `forecast_availability_score`: source-run availability, variable completeness, lead time, and run age for issued-forecast mode.
- `weather_feature_mode`: `historical_analysis` or `issued_forecast`; never silently compare the two as equivalent inputs.
...
1. Select perimeter, FIRMS, weather, and L2 tiles from candidate evidence
   available at the tile anchor/as-of time. Do not select a tile because of
   later spread. Historical weather is sampled at the resulting tile and
   hourly anchor, then marked `historical_analysis`; issued forecasts also
   require their captured availability gate.
...
| Open-Meteo Historical Weather API ECMWF IFS | Admit for the next historical rebuild | Retrieve compact hourly values only for FIRMS-seeded candidate tiles and their prediction anchors. Retain raw response, model, tile/candidate mapping, valid hour, and retrieval provenance as `historical_analysis`; do not call it an as-issued forecast. |
...
The HRDPS plan is deliberately not a claim that the historical forecast files were originally published at the specified anchor. It carries `forecast_availability_score=0.6` and a documented latency assumption. An issued-forecast value tile may only be joined to a training row after its model-run, valid time, source retrieval, and conservative availability policy pass the as-of check. That restriction does not prohibit the separately labelled Open-Meteo ECMWF IFS historical-analysis backfill.
